...
Calibrating the stereo camera involves determining the intrinsic and extrinsic parameters of the individual cameras. To accomplish this, we utilize the Matlab's [Stereo Camera Calibrator](https://www.mathworks.com/help/vision/ug/using-the-stereo-camera-calibrator-app.html). The checkerboard is used to calibrate the cameras by placing it at different orientations and distances with respect to the cameras. Capture multiple pairs of synchronized images of the [checkerboard](./misc/checkerboard.pdf) using both the left and right cameras. Make sure to cover various angles and distances to get a comprehensive set of data for calibration:
...
#### 2. **Attaching the Registration Unit**: 
- Attach the registration unit to the main part of the model to allow for the extraction of transformation data.

#### 3. **ROS Setup**:
- Ensure the ROS environment variables are properly set up to include paths to the AprilTag [definition](./misc/apriltag_definition.yaml) and [settings](./misc/apriltag_settings.yaml) files.

#### 4. **Launching the AprilTag Detection**:
...
The catheter, a pivotal component in the setup, is positioned to enable accurate tracking. Refer to the image below for a visualization of its placement:
